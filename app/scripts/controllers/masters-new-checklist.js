...
.controller('addItemOptionsModalInstance', function($scope, $log, $state, $uibModal, $uibModalInstance, $window, idItem, itemName, checklistName, ChecklistActions, infoChecklist, Utils) {
	$scope.modal = {
		checklist: []
	};
	...
	$scope.openModalAddOptions = function(id, name) 
	{
		var modalInstance = $uibModal.open({
			animation: true,
			templateUrl: 'addSubItemOptionsModal.html',
			controller: 'addSubItemOptionsModalInstance',
			resolve: {
				checklistName: function() {
					return checklistName;
				},
...
.controller('addSubItemOptionsModalInstance', function($scope, $log, $uibModal, $uibModalInstance, $window, idItem, itemName, idSubItem, nameSubItem, checklistName, ChecklistActions, infoChecklist, Utils) {
	...
	$scope.getInfoChecklist = function(e) {
		...
		for (i = 0; i < infoChecklist.items.length; i++) {
			if (infoChecklist.items[i].id === idItem) 
			{
				angular.forEach(infoChecklist.items[i].items, function(value)
				{
					if (value.id == idSubItem) 
					{
						$scope.modal.checklistItems = value.items;
					}
				});
			}
		}
		if ($scope.modal.checklistItems == undefined) 
		{
			$scope.modal.checklistItems = [];
		}
		$log.error($scope.modal.checklistItems);
	};
